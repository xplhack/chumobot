...
View_PORT(){
	...
	echo -e "$Ban_PORT_list" && echo && msg -bar3
}
View_KEY_WORDS(){
	Cat_KEY_WORDS
	echo -e "============${Red_background_prefix} Actualmente Prohibido ${Font_color_suffix}============"
	echo -e "$Ban_KEY_WORDS_list" && msg -bar3
}
View_ALL(){
	echo
	View_PORT
...
Ban_ALL(){
	...
	if [[ -z ${BT_KEY_WORDS} ]]; then
		...
		if [[ -z ${SPAM_PORT} ]]; then
			...
			echo -e "${Error} Torrent Bloqueados, Palabras Claves y Puertos SPAM,\nno es necesario volver a prohibir !" && msg -bar3 && exit 0
		fi
	fi
}
UnBan_BT(){
	check_BT
	[[ -z ${BT_KEY_WORDS} ]] && echo -e "${Error} Torrent y Palabras Claves no bloqueadas, verifique !"&& msg -bar3 && exit 0
	s="D"
	Set_BT
	View_ALL
...
UnBan_SPAM(){
	check_SPAM
	[[ -z ${SPAM_PORT} ]] && echo -e "${Error} Puerto SPAM no detectados, verifique !" && msg -bar3 && exit 0
	s="D"
	Set_SPAM
	View_ALL
...
UnBan_ALL(){
	...
	if [[ ! -z ${BT_KEY_WORDS} ]]; then
		...
		if [[ ! -z ${SPAM_PORT} ]]; then
			...
			echo -e "${Error} No se  detectan Torrent, Palabras Claves y Puertos SPAM Bloqueados, verifique !" && msg -bar3 && exit 0
		fi
	fi
}
...
UnBan_PORT(){
	...
	do
		View_PORT
		[[ -z ${Ban_PORT_list} ]] && echo -e "${Error} No se detecta puertos bloqueados !" && msg -bar3 && exit 0
		ENTER_UnBan_PORT
		Set_PORT
		echo -e "${Info} Puerto decapsulado [ ${PORT} ] !\n"
	done
...
UnBan_KEY_WORDS(){
	...
	do
		Cat_KEY_WORDS
		[[ -z ${Ban_KEY_WORDS_list} ]] && echo -e "${Error} No se ha detectado ningún bloqueo !" && msg -bar3 && exit 0
...
UnBan_KEY_WORDS_ALL(){
	Cat_KEY_WORDS
	[[ -z ${Ban_KEY_WORDS_text} ]] && echo -e "${Error} No se detectó ninguna clave, verifique !" && msg -bar3 && exit 0
	if [[ ! -z "${v6iptables}" ]]; then
...
$(msg -bar3)
 ${Green_font_prefix}12.${Font_color_suffix} Actualizar script" && msg -bar3
read -e -p " Por favor ingrese un número [0-12]:" num && msg -bar3
case "$num" in
	0)
	View_ALL
	;;
